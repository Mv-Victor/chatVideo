...
```bash
test -f ".codeflicker/agents/lisa.md" && echo "lisa.md: EXISTS" || echo "lisa.md: MISSING"
```
...
Missing: .codeflicker/agents/lisa.md
...
1. Spawn a fresh-context sub agent using the **Agent tool**:
   ...
   - **prompt**: Compose a prompt containing:
     - Instruct the agent to read and follow `.codeflicker/agents/lisa.md`
     - When those instructions reference a slash command (e.g., `/speckit.analyze`), read the corresponding folds and file from `.codeflicker/skills/` and follow its instructions directly
