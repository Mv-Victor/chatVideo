...
```bash
test -f ".codeflicker/agents/homer.md" && echo "homer.md: EXISTS" || echo "homer.md: MISSING"
```
...
Missing: .codeflicker/agents/homer.md
...
1. Spawn a fresh-context sub agent using the **Agent tool**:
   ...
   - **prompt**: Compose a prompt containing:
     - Instruct the agent to read and follow `.codeflicker/agents/homer.md`
     - When those instructions reference a slash command (e.g., `/speckit.clarify`), read the corresponding files and folds from `.codeflicker/skills/` and follow its instructions directly
