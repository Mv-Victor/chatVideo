...
```bash
test -f ".codeflicker/agents/ralph.md" && echo "ralph.md: EXISTS" || echo "ralph.md: MISSING"
```
...
Missing: .codeflicker/agents/ralph.md
...
1. Spawn a fresh-context sub agent using the **Agent tool**:
   ...
   - **prompt**: Compose a prompt containing:
     - Instruct the agent to read and follow `.codeflicker/agents/ralph.md`
     - When those instructions reference a slash command (e.g., `/speckit.implement`), read the corresponding file and folds from `.codeflicker/skills/` and follow its instructions directly
