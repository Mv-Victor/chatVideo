...
- Q: How does the manual track-volume adjustment feature (referenced in User Story 2 as "adjust track volumes") work in the data model, UI, and export pipeline — and what is the per-track volume field schema? → A: Per-track volume is stored as **top-level scalar fields** in `timeline.json` at the root object level alongside `tracks`: `video_volume` (float, 0.0–2.0, default 1.0), `voiceover_volume` (float, 0.0–2.0, default 1.0), and `bgm_volume` (float, 0.0–2.0, default 0.25). These fields are NOT stored on individual clip entries within the track arrays, because `node_schema.py` `ClipTrack`, `VoiceoverTrack`, and `BgmTrack` carry no `volume` attribute and `RenderVideoInput` accepts single scalar multipliers (not per-clip values). The timeline panel MUST render a per-track volume slider in each audio-bearing track header (V1, A1, A2). Adjusting the slider updates the corresponding top-level volume field in the in-memory timeline state, persists it to `timeline.json` via atomic write, and counts as a manual edit for undo/redo (FR-008). At export time, the backend reads `video_volume`, `voiceover_volume`, and `bgm_volume` from the top level of `timeline.json` and passes them to `RenderVideoInput` as `video_volume_scale`, `tts_volume_scale`, and `bgm_volume_scale` respectively; if any field is absent, the corresponding `RenderVideoInput` default is used (1.0 / 2.0 / 0.25). This corrects the prior answer's incorrect "per-clip volume on track array entries" model and eliminates the broken "use last entry" export heuristic. See updated FR-006 and FR-021 for normative requirements.
...
- **FR-006**: Timeline clips MUST be draggable (reorder), trimmable (in/out point
  ...
  per-track volume slider (range 0.0–2.0, default 1.0 for V1/A1 and 0.25 for A2,
  step 0.05) rendered in the track header area of the timeline panel. The adjusted
  volume value MUST be persisted as **top-level scalar fields** in `timeline.json`
  alongside the `tracks` object: `video_volume` (float, maps to
  `RenderVideoInput.video_volume_scale`), `voiceover_volume` (float, maps to
  `RenderVideoInput.tts_volume_scale`), and `bgm_volume` (float, maps to
  `RenderVideoInput.bgm_volume_scale`). These fields are NOT stored on individual
  clip entries within the track arrays, because the actual backend `node_schema.py`
  `ClipTrack`, `VoiceoverTrack`, and `BgmTrack` models carry no `volume` attribute
  and `RenderVideoInput` accepts single scalar volume parameters (see FR-021 for
  the full `timeline.json` schema). When `POST /projects/{project_id}/export` is
  called, the backend MUST read `video_volume`, `voiceover_volume`, and `bgm_volume`
  from the top level of `timeline.json` and pass them to `RenderVideoInput` as
  `video_volume_scale`, `tts_volume_scale`, and `bgm_volume_scale` respectively;
  if any field is absent the corresponding `RenderVideoInput` default MUST be used
  (1.0 / 2.0 / 0.25). Volume adjustment on the timeline MUST be treated as a manual
  edit for undo/redo purposes (FR-008).
...
- **FR-021**: The canonical Timeline JSON schema used by `timeline.json`, `timeline_update` WebSocket events, and the `/preview/frame` and export endpoints is derived from the backend's `src/open_storyline/nodes/node_schema.py` Pydantic models. All time values are integers representing **milliseconds**. Per-track volume is stored as **top-level scalar fields** at the root of the `timeline.json` object (alongside `tracks`), NOT on individual clip entries, because `node_schema.py` `ClipTrack`, `VoiceoverTrack`, and `BgmTrack` carry no `volume` attribute and `RenderVideoInput` accepts single scalar multipliers. The schema is:
  ...
  ```json
  {
    ...
    "voiceover_volume": "<float — per-track voiceover volume multiplier; range 0.0–2.0, default 1.0; maps to RenderVideoInput.tts_volume_scale at export>",
